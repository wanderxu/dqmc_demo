...
!! r = spring_mt_stream() ! range (0,1)
!! r = spring_mt_string() ! range [0,1]
!!
!! 2. use SFMT
!! -----------
!!
...
!! r = spring_sfmt_stream() ! range (0,1)
!! r = spring_sfmt_string() ! range [0,1]
!!
!! note: since SFMT has a better performance, it is preferable
!!
!! 3. utilities based on above pseudorandom number generator
!! -----------
!!
!! function spring_sfmt_gaussian(mu, sigma) result(r) ! random number with gaussian distribution
!!
!! 
!! original author  : li huang (email:huangli712@gmail.com)
!! modified by Xiao Yan Xu (email:wanderxu@gmail.com)
  ...
  module spring
     ...
     integer(int32), private, parameter :: N = 624_int32 ! period parameters
     integer(int32), private, parameter :: M = 397_int32 ! period parameters

     integer(int32), private, parameter :: ME  = 19937 ! Mersenne Exponent. the period of the sequence is a multiple of 2^{ME}-1
     integer(int32), private, parameter :: NS  = ME / 128 + 1 ! SFMT has an internal state array of 128-bit integers, and NS is its size.
     integer(int32), private, parameter :: N32 = NS * 4 ! N32 is the size of internal state array when regarded as an array of 32-bit integers
     integer(int32), private, parameter :: N64 = NS * 2 ! N64 is the size of internal state array when regarded as an array of 64-bit integers

     integer(int32), private, parameter :: parity(0:3) = (/1, 0, 0, 331998852/) ! a parity check vector which certificate the period of 2^{ME}

     integer(int32), private, save :: mti = -1 ! states pointer, if mti == -1, itialization is necessary
     integer(int64), private, save :: mt(0:N-1) ! states array for twist generator

     integer(int32), private, save :: idx = -1 ! index counter to the 32-bit internal state array
     integer(int32), private, save :: pt32(0:N32-1) ! the 32-bit integer pointer to the 128-bit internal state array
     integer(int64), private, save :: pt64(0:N64-1) ! the 64-bit integer pointer to the 128-bit internal state array
  ...
  !! spring_mt_init: initializes the MT19937 generator with "seed"
     ...
     integer(int32), intent(in) :: seed ! seed for random number generator
  ...
  !! (0,1), i.e., a number greater than 0 and less than 1
     implicit none
     real(ieee64), parameter :: factor = 1.0_ieee64 / 4294967296.0_ieee64 ! pre-calculated to avoid division below
  ...
  !! equal to 1
     implicit none
     real(ieee64), parameter :: factor = 1.0_ieee64 / 4294967295.0_ieee64 ! pre-calculated to avoid division below
     real(ieee64) :: r ! return type
  ...
  function spring_sfmt_gaussian(mu, sigma) result(r)
  !! use polar form of the Box-Muller transformation to Generating Gaussian Random Numbers
  !! ( with zero mean and standard deviation one ).
    implicit none
    real(ieee64), intent(in) :: mu
    real(ieee64), intent(in) :: sigma
    real(ieee64) :: r
    real(ieee64) :: x1, x2, w, y1, y2
    w = 0.0_ieee64
    do while ( w .ge. 1.0_ieee64 .or. w .eq. 0.0_ieee64 )
