subroutine mmuur(a_up, a_dn, nf, ntau, nflag)
  ...
  if ( nflag.eq.2 ) then
     do i = 1,lq
        ...
        if (nf.gt.4) then
           !current.
           do j = 1,ndim
              a_up(i1,j) = xsigp2(nsigl_j(i1,nn,ntau)) * v1(j)
              a_up(i2,j) = xsigm2(nsigl_j(i1,nn,ntau)) * v2(j)
           enddo
        else
           !kinetic
           do j = 1,ndim
              a_up(i1,j) = xsigp2(nsigl_k(i1,nn,ntau))*v1(j)
              a_up(i2,j) = xsigm2(nsigl_k(i1,nn,ntau))*v2(j)
           enddo
        endif
     enddo
  endif
