subroutine mmuulm1(a_up, a_dn, nf, ntau, nflag)
...
!$OMP END PARALLEL
  ...
  if (nflag.eq.2) then
      do i = 1,lq
        ...
        if (nf.gt.4) then ! current.
           do j = 1,ndim
               a_up(j,i1) = v1(j) / xsigp2(nsigl_j(i1,nn,ntau))
               a_up(j,i2) = v2(j) / xsigm2(nsigl_j(i1,nn,ntau))
           enddo
        else              ! kenitic
           do j = 1,ndim
               a_up(j,i1) = v1(j) / xsigp2(nsigl_k(i1,nn,ntau))
               a_up(j,i2) = v2(j) / xsigm2(nsigl_k(i1,nn,ntau))
           enddo
        endif
      enddo
  endif
